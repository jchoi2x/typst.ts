...
} from '@jchoi2x/typst.ts/src/fs/package.worker.mjs';
...
export class TypstCompilerManager {
  ...
  private readonly fontBytesPromise: Promise<[Uint8Array, Uint8Array]> = Promise.all([
  ...
  private async fetchRenderCvFontUrls(family: string): Promise<string[]> {
    ...
    try {
      ...
      console.warn(`[TypstCompilerManager] rendercv font lookup failed for "${family}":`, error);
      this.renderCvFamilyUrlCache.set(normalized, []);
      return [];
    }
  }
  ...
  private loadFontMapFromEnv(): void {
    ...
    try {
      ...
      console.warn('[TypstCompilerManager] invalid FONT_FAMILY_URLS_JSON:', error);
    }
  }
      ...
      this.compilerPromise = (async () => {
        ...
        for (const url of this.dynamicFontUrls.values()) {
          try {
            ...
            console.warn(`[TypstCompilerManager] skipping dynamic font URL (${url}): ${message}`);
          }
        }
  ...
  async compilePdfResponse(maxRetries: number, sample_type: string): Promise<Response> {
