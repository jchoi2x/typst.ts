// @ts-ignore
import type * as typst from '@jchoi2x/typst-ts-renderer';
...
export async function rendererBuildInfo(): Promise<any> {
  const renderModule = await import('@jchoi2x/typst-ts-renderer');
...
export class TypstRendererDriver {
  ...
  async init(options?: Partial<InitOptions>): Promise<void> {
    this.rendererJs = await (options?.getWrapper?.() || import('@jchoi2x/typst-ts-renderer'));
