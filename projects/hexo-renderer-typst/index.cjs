...
hexo.extend.generator.register('typst_assets', function (locals) {
  ...
  const typst_main_path = path.resolve(
    base_dir,
    'node_modules/@jchoi2x/typst.ts/dist/esm/main.bundle.js',
  );
  ...
  if (!fs.existsSync(typst_main_path)) {
    throw new Error('typst.ts not found, please install `@jchoi2x/typst.ts` first');
  }

  if (!fs.existsSync(renderer_path)) {
    throw new Error(
      'typst-ts-renderer not found, please install `@jchoi2x/typst-ts-renderer` first',
    );
  }
